...
13. **报错要把「0 个」和「≥2 个」分开说**。两者是**不同的病**：0 个 = 地垫与房间不同源（生成侧
    ...
    （本段实测按错方向查了两轮）。
14. **"房间多边形非法"不能一刀切成"非法即拦"**：全库 **558 个自交多边形、分布在 22 栋**
    （c006 51 / c017 250 / c059 72 / c034 48 / c104 20 / c009 17 / c080 17 / ny27 12 / c103 11…），
    其中 **22 栋现在全部通过**管道 —— GEOS 只在**恰好撞上**某个运算时才抛。只能把抛出的那一刻
    指名道姓报出来（`_geos_meas`），不能整层拦。
...
- **SU 分层建模（`su_spec_floors_fleet.py`）全库 50 栋：49 通过，c103 未通。**
  未通的**唯一**原因是 `floors/` 里 11 个房间多边形**自交**（F1×2、F2–F4 各 3），
  GEOS 判地垫归属时抛 TopologyException；沙盘实测（`_scratch/_probe_c103_fixable.py`：
  复制到 `_scratch/_tmp_c103_fixprobe/` 修自交后重跑）**修完即通过**，代价是
  8 间面积不变、3 间各 −0.345 m²（合计 −1.04 m²，−0.009%）。**尚未动手，等定夺。**
- **`floors/` 里房间嵌套/同号重复的楼 4 栋**（`_scratch/_probe_room_nesting.py` 普查）：
  c103（A 翼 `103-A-0N-02` 每层**两条同号房**：1537 m² 与 3422 m²，且 1537 那条把 B 翼小房
  整个包住）、c041 与 c034（`4N-0N-01` 各一条 80.1 m² 包住 74.7 m² 同号房，差 5.46 m²
  ≈ 0.15 m 外墙带）、c033（`34-01-05A` 602.5 m² 包住 `34-01-11` 20.4 m²）。
  它们**不拦管道**（判据① 逐块精确命中自家房间就短路了），但会让 SU 画出**共面重叠地垫**
  （同 z、100% 重叠 ⇒ 闪面）。c041 的"逐点完全相同"副本已删 41 条
  （`_scratch/_dedup_floor_rooms.py`，备份在 `.orig/floors.before_dedup/`），
  **同号嵌套这一类比"重复副本"更宽，尚未处置。**
