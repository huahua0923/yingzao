...
14. **"房间多边形非法"不能一刀切成"非法即拦"**：全库 **558 个自交多边形、分布在 21 栋**
    ...
    不能整层拦。2026-09-13 已修 **532 间**（19 栋 / 65 层，`_scratch/_fix_self_intersections.py`），
    **26 间**保留原样（c006 13 / c103 11 / c009 1 / c041 1，见「已知缺口」）。
...
- **SU 分层建模（`su_spec_floors_fleet.py`）全库 50 栋：49 通过，c103 未通。**
  ...
  复制到 `_scratch/_tmp_c103_fixprobe/` 修自交后重跑）**修完即通过**，代价 8 间面积不变、
  3 间各 −0.345 m²（合计 −1.04 m²，−0.009%）。**c103 这 11 间仍未修**：其中 4 间正是 A 翼
  那条证据冲突的同号房（见下），先改它等于替人工把 A 翼的账做了一半 —— 等定夺。
- **全库房间多边形自交已修 532 间（19 栋 / 65 层）**（`_scratch/_fix_self_intersections.py`，
  备份 `.orig/floors.before_selfint/`，65 个层文件）：全部是**区域逐点不变**的清洗 ——
  与改前逐间比对，对称差 ≤1e-6 m²、房间**条数**不变（`_scratch/_probe_fix_scope.py`）。
  **26 间保留原样**（c006 13 / c103 11 / c009 1 / c041 1，逐间清单
  `_scratch/_qa_selfint/remaining_invalid.txt`）：
  ...
  - c041 `41-06-04` 1 间；c103 的 11 间见下条。
  **对 SU 产物零影响，且这次是实测的不是推的**：全库 **49 份 spec 与改前逐字节相同**
  （`_scratch/_probe_spec_census_diff.py` → `_qa_selfint/spec_census_diff2.txt`）；
