...
14. **"房间多边形非法"不能一刀切成"非法即拦"**：全库 **558 个自交多边形、分布在 21 栋**
    ...
    不能整层拦。2026-09-13 已修 **543 间**（20 栋 / 69 层，`_scratch/_fix_self_intersections.py`），
...
- **全库房间多边形自交已修 543 间（20 栋 / 69 层）**（`_scratch/_fix_self_intersections.py`，
  备份 `.orig/floors.before_selfint/`，69 个层文件）：532 间是**区域逐点不变**的清洗 ——
