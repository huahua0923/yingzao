...
function paintTotals() {
  ...
  return el('section', { class: 'b-totals' },
    ...
    nRooms > 0 ? el('p', { class: 'b-note dim', text: roomArea > 0
      ...
      // ★ 全为 0 时必须说清"0 是这个意思"。全库 22 栋逐层 0 间（实测），
      //   成因至少两种：图上本来没有房号（体育场/球场），或这栋楼还没进房间抽取的
      //   名单。两者在屏幕上都是"0 间" —— 而 0 是个**断言**（真的一间都没有），
      //   不是"不知道"。前台不猜成因，但要把"没台账"这件事说出来，并指明
      //   逐栋结论在后台「检查」页（记忆：模板串印出 undefined ——
      //   "不知道"的正确写法是不说，而不是说成 0）。
      : (state.floors.length ? el('p', { class: 'b-note dim',
          text: '逐层房间计数全为 0 —— 这栋楼没有房间台账。是图上没有房号、'
              + '还是还没抽，后台「检查」页有逐栋结论，这里不猜。' }) : null));
}
