...
ID_BASE = {
    ...
    #   ★ 上面这个 45 是**补名单之前**的测量（2026-09-23 夜）。补完之后复量是
    #     22 栋空台账 / 18 栋不在本表（2026-09-24 凌晨）。三个数都不是现况 ——
    #     现况跑 `python backend/checks/runner.py`，看 A1 与 A6 的行，
    #     别在这条注释里找答案（memory: comment-describing-past-run-goes-stale）。
    # 名单准入判据（缺一不可，逐栋核过）：
    ...
    #   · c004f1 c004f2 —— **模型只建了一翼**（2026-09-24 凌晨补记；这两栋此前
    #     **一条理由都没写**，看着就像"漏登"，所以特意写明）。
    #     实测 A7 的「模型面积 ÷ 图纸面积」（`runner.py c004f1` 自己算的）：
    #       c004f1  模型 282/98/94/51㎡ 对图纸 731/713/713/365㎡ —— 中位比 **0.14**
    #       c004f2  模型 47㎡ 对图纸 517㎡                              —— 比 **0.09**
    #     ⇒ **不许登进本表**：登进去就会照**整栋**的图纸抽出满楼房间，而模型只有
    #       一翼，绝大多数房间落在轮廓外（c072/c073 那种"几百间房放不进去"的病）。
    #     ★ 这两栋的**层数其实是对的**（图纸 4 层 vs 模型 4 层；1 vs 1，实测），
    #       所以"层数对得上"这一条**不足以**让一栋进名单 —— 还得看**平面范围**。
    #     ★ 反过来，"名字带 f 后缀"也**不足以**判定分片：同批 c008f1 名字一样像
    #       分片，实测比值 0.58/1.01（**同数量级，不是分片**），它本来就在表里。
    #       ⇒ 是不是分片**只能量**（A7 那个比值），不能看名字。
    #       （builtin.py 的 `_fragment_base()` 就是按名字猜的那个提示，A7 用它只为
    #        给异常层加一句说明，不影响任何状态 —— 本条的比值是独立量出来的。）
