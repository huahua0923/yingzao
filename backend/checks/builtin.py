    ...
    if n == 0:
        ...
        if table is not None:
            ...
            if name not in table:
                ...
                #   （模型 282/98/94/51㎡ 对图纸 731/713/713/365㎡），**只建了一翼**；
                #   登进去会把**整栋图纸**的房间铺到一翼模型上，绝大多数落在轮廓外
                #   （就是 c072/c073 那种"几百间房放不进去"的病）。
                #   下面这句只把"先量一把"指出来，**不替人判死活**：
                #   名字像分片（`*fN` 且母栋确实在库里）只是提示，不是证据 ——
                #   同批的 c008f1 名字一样像分片，实测比值 0.58/1.01，**不是**分片。
                frag = _fragment_base(data_dir, name)
                if frag:
                    ev["fragment_base"] = frag
                    why += ("。★ **但先别急着登**：本栋名字像是 `%s` 的分片，"
                            "而 `%s` 也在库里 —— 分片登名单是有害的（房间会铺满"
                            "整栋图纸，模型只有一翼）。登之前先量一把（只读）："
                            "`python -u backend/checks/runner.py %s` 看 **A7** 的"
                            "「模型面积 ÷ 图纸面积」—— **远小于 1 就是分片，不该登**；"
                            "与图纸同数量级才是真缺名单。★ 若那条 **A7 自己也报 "
                            "`unavailable`**（该栋没跑过对账、或图上没有面积表），"
                            "那就是**还没法判**：此时既不该登、也不该猜，先把 A7 "
                            "跑出来再说（c012f1 实测就是这一档）。★ 名字只是提示、"
                            "不是证据：同一批的 c008f1 名字同样像分片，实测比值 "
                            "0.58/1.01，**不是**分片" % (frag, frag, name))
            else:
                # ★ 在名单里却仍空 —— 这一档**不是名单的问题**，而上面那句默认话
                #   还写着「先查是否在 ID_BASE 里（见 A6）」，等于把人指去查一个
                #   本条刚刚查完、且 A6 会报 pass 的答案：**一句话盖两种情形，
                #   对一种是对的、对另一种是死胡同**（与 A8 那处同族）。
                #   这一档只有一把尺，且是只读的（A 层只有标准库，不能直接调抽取器，
                #   所以指向命令而不是自己跑）—— 顺带把「跑过没命中」与「压根没跑过」
                #   分开，这两者在屏幕上是同一个空台账。
