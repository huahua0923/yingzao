...
用法
----
    python backend/checks/_defect_census.py                 # 全部 49 栋
    python backend/checks/_defect_census.py c084 c009 c055  # 指定
"""
import json
import os
import sys

#: ★ 2026-09-24 收编时改：本文件从 `_scratch/` 搬到 `backend/checks/`，**深了一层**，
#:   所以 `dirname` 要多一次才回到仓库根。原来两层是 `_scratch → 根`，现在三层是
#:   `checks → backend → 根`。少这一层不会报错，只会把 `backend/` 当成仓库根 ——
#:   于是数据目录找不到、路径全错，而屏幕上看着像"这栋楼没数据"。
